...
class DrivingCalculateController: UIViewController, AMapNaviDriveManagerDelegate, UICollectionViewDelegateFlowLayout {
	...
	// MARK: - Initalization
	func initMapView() {
		mapView = MAMapView().then({
			$0.frame = CGRect(x: 0, y: 0, width: view.bounds.width, height: view.bounds.height - routePlanInfoViewHeight)
			$0.delegate = self
			view.addSubview($0)
		})
	}
	...
	func initRouteIndicatorView() {
		...
		guard let routeIndicatorView = routeIndicatorView else { return }
	...
	func addAnnotations() {
		let beginAnnotation = NaviPointAnnotation().then({
			$0.coordinate = CLLocationCoordinate2D(latitude: Double((startPoint?.latitude)!), longitude: Double((startPoint?.longitude)!))
			$0.title = "起始点"
			$0.naviPointType = .start
		})

		let endAnnotation = NaviPointAnnotation().then({
			$0.coordinate = CLLocationCoordinate2D(latitude: Double((endPoint?.latitude)!), longitude: Double((endPoint?.longitude)!))
			$0.title = "终点"
			$0.naviPointType = .end
		})
	...
	func showNaviRoutes() {
		guard let allRoutes = driverManager?.naviRoutes else { return }
	...
	func selecteOverlayWithRouteID(routeID: Int) {
		guard let allOverlays = mapView.overlays else {	return }
		
		for (index, aOverlay) in allOverlays.enumerated() {
			if let selectableOverlay = aOverlay as? SelectableOverlay {
				
				guard let overlayRenderer = mapView.renderer(for: selectableOverlay) as? MAPolylineRenderer else { return }
	...
	func configSubview() {
		...
		preferenceView = PreferenceView().then({
			$0.frame = CGRect(x: 0, y: 60, width: view.bounds.width, height: 30)
			$0.isHidden = true
			view.addSubview($0)
			//进行多路径规划
			driverManager?.calculateDriveRoute(withStart: [startPoint!],
			                                   ...
			                                   wayPoints: nil,
			                                   drivingStrategy: $0.strategy(isMultiple: true))
		})
	}
...
extension DrivingCalculateController: MAMapViewDelegate {
	...
	func mapView(_ mapView: MAMapView!, rendererFor overlay: MAOverlay!) -> MAOverlayRenderer! {
		if overlay is SelectableOverlay {
			...
			let polylineRenderer = MAPolylineRenderer(overlay: selectableOverlay.overlay).then({
				$0.lineWidth = 8.0
				$0.strokeColor = selectableOverlay.selected ? selectableOverlay.selectedColor : selectableOverlay.reguarColor
			})
